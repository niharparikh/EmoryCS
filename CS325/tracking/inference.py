    ...
    def observe(self, observation, gameState):
        ...
        for p in self.legalPositions:
            if noisyDistance == None:
                ...
                true_distance = util.manhattanDistance(pacmanPosition, p)
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for next_pos, prob in newPosDist.items():
                allPossible[next_pos] += prob * self.beliefs[p]
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for p in self.particles:
                particle_distance = util.manhattanDistance(pacmanPosition, p)
                allPossible[p] += emissionModel[particle_distance]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        permutations = list(itertools.product(self.legalPositions, repeat=num_ghosts))
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for p in self.particles:
            ...
            for ghost_index in range(num_ghosts):
                if noisyDistances[ghost_index] == None:
                    p = self.getParticleWithGhostInJail(p, ghost_index)
                else:
                    particle_distance = util.manhattanDistance(pacmanPosition, p[ghost_index])
                    weight = weight * emissionModels[ghost_index][particle_distance]
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            num_ghosts = self.numGhosts
            for ghost_index in range(num_ghosts):
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), ghost_index,
