...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        # Find most likely and closest position of living ghosts
        for distribution in livingGhostPositionDistributions:
            likely_position = distribution.argMax()
            temp_distance = self.distancer.getDistance(pacmanPosition, likely_position)
            if distance_current > temp_distance:
                distance_current = temp_distance
                closest_ghost_position = likely_position

        # Get action that brings Pacman closer to that ghost position
        for action in legal:
            successor = Actions.getSuccessor(pacmanPosition, action)
            temp_distance = self.distancer.getDistance(closest_ghost_position, successor)
            if distance_next > temp_distance:
                distance_next = temp_distance
                best_action = action
