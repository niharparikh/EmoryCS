...
class QLearningAgent(ReinforcementAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        previous_q = self.q_values[(state, action)]
        updated_q = ((1 - self.alpha) * previous_q) + (self.alpha * (reward + (self.discount * next_val)))
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        features = self.featExtractor.getFeatures(state, action)
        previous_q = self.getQValue(state, action)
        q_val = self.computeValueFromQValues(nextState)

        difference = reward + (self.discount * q_val) - previous_q
